...
public class ContinuousGFXUISelector extends GFXUISelector implements ActionSelector, Serializable
{
	...
	{ Action.UPLEFT, Action.UP, Action.UPRIGHT, Action.LEFT, Action.SELF, Action.RIGHT, Action.DOWNLEFT, Action.DOWN,
			Action.DOWNRIGHT };
	...
	{
		QCURSORS = new Cursor[]
		{ assets.getCursorAsset("QUADRANT_0_CURSOR"), assets.getCursorAsset("QUADRANT_1_CURSOR"),
				assets.getCursorAsset("QUADRANT_2_CURSOR"), assets.getCursorAsset("QUADRANT_3_CURSOR"),
				assets.getCursorAsset("QUADRANT_4_CURSOR"), assets.getCursorAsset("QUADRANT_5_CURSOR"),
				assets.getCursorAsset("QUADRANT_6_CURSOR"), assets.getCursorAsset("QUADRANT_7_CURSOR"),
	...
	protected MouseListener getMouseClickListener()
	{
		...
		return new MouseAdapter()
		{
			public void mousePressed(final MouseEvent e)
			{
				...
				{
					// logger.debug("left mouse pressed");
					mousePosition = e.getPoint();
					int quadrant = defineQuadrant(mousePosition.x, mousePosition.y);
					selectedMouseDirection = QDIRECTIONS[quadrant - 1];
				...
				{
					// logger.debug("right mouse pressed");
	...
	{
		return selectedCharCode != -1 || selectedMouseDirection != -1 || selectedMousePosition != null;
	}
	...
	{
		Actor vMonster = player.getLevel()
				.getActorAt(Position.add(player.getPosition(), Action.directionToVariation(direction)));
