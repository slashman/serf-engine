...
public class GridLevelReader extends AbstractLevel
{
	...
	public GridLevelReader(String levelNameset, int levelWidth, int levelHeight, int gridWidth, int gridHeight,
		...
		try
		{
			...
			for (int y = 0; y < map.length; y++)
			{
				for (int x = 0; x < map[0].length(); x++)
				{
					...
					if (iconic == null)
						SworeGame.crash("mapchar " + map[y].charAt(x) + " not found on the level charMap",
								new Exception());
					...
					if (cmds.length > 1)
					{
						...
						else if (cmds[1].equals("EXIT_ABS_FEATURE"))
						{
							addExit(new Position(where.x + x, where.y + y, where.z), cmds[2]);
