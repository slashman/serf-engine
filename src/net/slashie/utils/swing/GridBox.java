...
public class GridBox extends JPanel {
	...
	public GridBox(SwingSystemInterface g, final int itemHeight, final int itemWidth, final int gridX, final int gridY){
		...
		setSize(gridX * itemWidth, gridY * itemHeight + legendLines * lineHeight);
		// Mouse things
	...
	protected SelectedItem getSelectedItemByClick(Point point, int legendLines, int lineHeight) {
		...
		pixelX -= getLocation().x;
		pixelY -= getLocation().y + (legendLines + 2) * lineHeight;
	...
	public void draw(boolean refresh){
		...
		// Draw the footer
		if (pages > 1){
			si.printAtPixel(getDrawingLayer(), (int)getLocation().getX(), (int)getLocation().getY()+getHeight()-lineHeight, "Page "+(currentPage+1)+"/"+pages, Color.WHITE);
		}
		if (refresh)
			si.commitLayer(getDrawingLayer());
	}
